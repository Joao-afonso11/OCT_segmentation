...
class SeUNet_multi(nn.Module):
    def __init__(self, in_channels=3, out_channels=1):
        super(SeUNet_multi, self).__init__()
        ...
        self.out_conv = nn.Conv2d(64, out_channels, kernel_size=1)  # Output with 1 channel (grayscale)
    ...
    def forward(self, x):
        ...
        x = torch.cat([x, self.se_block3(x5)], dim=1)  # Adding SE block for skip connection
        x = self.up_conv1(x)
        x = self.up_transpose2(x)
        x = torch.cat([x, self.se_block2(x3)], dim=1)  # Adding SE block for skip connection
        x = self.up_conv2(x)
        x = self.up_transpose3(x)
        x = torch.cat([x, self.se_block1(x1)], dim=1)  # Adding SE block for skip connection
        x = self.up_conv3(x)
        x = self.out_conv(x)  # Output with 1 channel (grayscale)
        return x
