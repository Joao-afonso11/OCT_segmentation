...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, use_large_kernel=False):
        super(DoubleConv, self).__init__()
        k = 7 if use_large_kernel else 3
        p = 3 if use_large_kernel else 1
...
class DRUNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # Encoder
        self.down_conv1 = DoubleConv(in_channels, 64, use_large_kernel=True)  
    ...
    def forward(self, x):
        x1 = self.se_block1(self.down_conv1(x))
        x2 = self.max_pool(x1)
        x3 = self.se_block2(self.down_conv2(x2))
        x4 = self.max_pool(x3)
        x5 = self.se_block3(self.down_conv3(x4))
        x6 = self.max_pool(x5)
        x7 = self.se_block4(self.down_conv4(x6))
